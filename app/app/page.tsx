...
export default function App() {
  ...
  function resetAll() {
    setFile(null); setYoutubeUrl(""); setResult(null); setActiveClip(null);
    ...
    try {
      ...
      if (mode === "upload") {
        ...
        res = await fetch("/api/process-video", {
          ...
          body: JSON.stringify({ url: youtubeUrl, duration, format }),
        });
      }
    ...
    <>
      <style>{`
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .format-label { display:flex; justify-content:space-between; font-size:13px; color:rgba(255,255,255,0.5); margin-bottom:10px; }
        .format-label strong { color:#b57bee; font-size:14px; }
        ...
        .vip-btn { background:linear-gradient(135deg,#7c3aed,#c026d3); border:none; color:#fff; padding:17px; border-radius:12px; font-size:16px; font-weight:700; cursor:pointer; transition:all .2s; animation:btn-pulse 2.5s ease-in-out infinite; }
      ...
      <div className="page">
        ...
        <section className="hero">
          <div className="badge"><span className="badge-dot" /> Gere seus clips agora</div>
          <h1>Gere clips <em>virais</em><br />em segundos.</h1>
          <p className="hero-sub">Faça upload do seu vídeo ou cole um link do YouTube e gere <strong>clips prontos para TikTok, Reels e Shorts</strong> automaticamente.</p>
          <div className="hero-btns">
            ...
            <button className="btn-ghost" onClick={() => window.location.href="/"}>← Voltar para o início</button>
          </div>
        </section>
        ...
        <div className="tool-wrap" id="tool" ref={toolRef}>
          <div className="tool-card">
            ...
            <div className="format-label">
              <span>Formato do clip</span>
              <strong>{format === "9:16" ? "📱 9:16 Vertical" : "📺 Original"}</strong>
            </div>
            <div className="format-grid" style={{marginBottom:20}}>
              <button className={`dur-btn${format === "original" ? " on" : ""}`} onClick={() => setFormat("original")} disabled={loading}>📺 Original</button>
              <button className={`dur-btn${format === "9:16" ? " on" : ""}`} onClick={() => setFormat("9:16")} disabled={loading}>📱 9:16 Vertical</button>
            </div>
        ...
        {result?.success && result.clips && result.clips.length > 0 && (
          <div className="results-wrap" ref={resultsRef}>
            <div className="results-top">
              ...
              <p className="results-meta">{Math.floor((result.totalSeconds ?? 0) / 60)}min · {formatDuration(result.clipDuration ?? 30)} · {result.format === "9:16" ? "📱 9:16" : "📺 Original"}</p>
        ...
        {result && !result.success && (
          <div className="err-box">
            ...
            <div style={{marginTop:14}}><button className="novo-btn" onClick={resetAll}>+ Tentar com outro vídeo</button></div>
          </div>
        )}
            ...
            <div className="countdown-grid">
              {[{n:countdown.days,l:"Dias"},{n:countdown.hours,l:"Horas"},{n:countdown.minutes,l:"Min"},{n:countdown.seconds,l:"Seg"}].map((c,i) => (
                <div key={i} className="countdown-block">
                  <span className="countdown-num">{String(c.n).padStart(2,"0")}</span>
