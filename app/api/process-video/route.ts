...
export async function POST(req: NextRequest) {
  ...
  // ── MODO 2: URL DO YOUTUBE via YTStream RapidAPI ──────────────────────────
    ...
    if (!videoId) {
      return NextResponse.json({ error: "ID do vídeo não encontrado na URL" }, { status: 400 });
    }
    ...
    try {
      ...
      if (!apiRes.ok) {
        ...
        return NextResponse.json({ error: "Falha na API YTStream", detail: errText }, { status: 500 });
      }
      ...
      else return NextResponse.json({ error: "Nenhum formato MP4 encontrado", detail: JSON.stringify(apiData).slice(0, 300) }, { status: 500 });
    } catch (err: unknown) {
      const e = err as { message?: string };
    ...
    try {
      ...
      if (!videoRes.ok) {
        return NextResponse.json({ error: "Falha ao baixar vídeo", detail: `Status ${videoRes.status}` }, { status: 500 });
      }
  ...
  // Filtro de vídeo baseado no formato escolhido
  const videoFilter = format === "9:16"
    ? `-vf "crop=ih*9/16:ih:(iw-ih*9/16)/2:0"`
    : "";

  // Se tem filtro de vídeo, não pode usar -c:v copy
  ...
  for (let i = 0; i < totalClips; i++) {
    ...
    try {
      ...
      if (fs.existsSync(clipPath)) {
        ...
        clips.push({ index: i + 1, clipUrl: `/api/video/${clipFilename}`, clipFilename, start, end: start + clipDuration, sizeKB });
      }
    } catch (err) {
