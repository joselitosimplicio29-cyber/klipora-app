...
export async function POST(req: NextRequest) {
  ...
  if (contentType.includes("multipart/form-data")) {
    ...
    try { formData = await req.formData(); } catch { return NextResponse.json({ error: "Erro ao ler FormData" }, { status: 400 }); }
    ...
    if (!videoUrl || !videoUrl.startsWith("http")) return NextResponse.json({ error: "URL invalida" }, { status: 400 });
    const dur = body?.duration ?? 30;
    clipDuration = ALLOWED.includes(dur) ? dur : 30;
    format = body?.format ?? "original";
    const RAPIDAPI_KEY = process.env.RAPIDAPI_KEY;
    if (!RAPIDAPI_KEY) return NextResponse.json({ error: "RAPIDAPI_KEY nao configurada" }, { status: 500 });
    const ytMatch = videoUrl.match(/(?:v=|youtu\.be\/|shorts\/)([a-zA-Z0-9_-]{11})/);
    const videoId = ytMatch?.[1];
    if (!videoId) return NextResponse.json({ error: "ID do video nao encontrado" }, { status: 400 });
    let downloadUrl = "";
    try {
      const apiRes = await fetch(`https://ytstream-download-youtube-videos.p.rapidapi.com/dl?id=${videoId}`, {
        method: "GET",
        headers: { "x-rapidapi-host": "ytstream-download-youtube-videos.p.rapidapi.com", "x-rapidapi-key": RAPIDAPI_KEY },
      });
      if (!apiRes.ok) { const errText = await apiRes.text(); return NextResponse.json({ error: "Falha na API YTStream", detail: errText }, { status: 500 }); }
      const apiData = await apiRes.json();
      const formats = apiData?.formats || apiData?.adaptiveFormats || [];
      const mp4WithAudio = formats.find((f: { mimeType?: string; url?: string; audioQuality?: string }) => f.mimeType?.includes("video/mp4") && f.url && f.audioQuality);
      const mp4Any = formats.find((f: { mimeType?: string; url?: string }) => f.mimeType?.includes("video/mp4") && f.url);
      ...
      else return NextResponse.json({ error: "Nenhum formato MP4 encontrado" }, { status: 500 });
    } catch (err: unknown) {
      const e = err as { message?: string };
    ...
    try {
      ...
      if (!videoRes.ok) return NextResponse.json({ error: "Falha ao baixar video", detail: `Status ${videoRes.status}` }, { status: 500 });
      const buffer = Buffer.from(await videoRes.arrayBuffer());
      fs.writeFileSync(videoPath, buffer);
    } catch (err: unknown) {
      const e = err as { message?: string };
      return NextResponse.json({ error: "Erro ao salvar video", detail: e?.message }, { status: 500 });
    }
  } else {
    return NextResponse.json({ error: "Content-Type nao suportado" }, { status: 400 });
  }
  if (!fs.existsSync(videoPath)) return NextResponse.json({ error: "Arquivo nao encontrado" }, { status: 500 });
  let totalSeconds = 0;
  try {
    ...
    return NextResponse.json({ error: "Falha ao detectar duracao", detail: e?.stderr?.toString?.() || e?.message }, { status: 500 });
  }
  if (totalSeconds < clipDuration) {
    try { fs.unlinkSync(videoPath); } catch { }
    return NextResponse.json({ error: `Video muito curto (${totalSeconds}s) para clips de ${clipDuration}s` }, { status: 400 });
  }
  ...
  for (let i = 0; i < totalClips; i++) {
    ...
    try {
      ...
      if (fs.existsSync(clipPath)) {
        ...
        clips.push({ index: i + 1, clipUrl: publicUrl, clipFilename, start, end: start + clipDuration, sizeKB });
        try { fs.unlinkSync(clipPath); } catch { }
      }
    } catch (err) { console.error(`Erro no clip ${i + 1}:`, err); }
  }
  try { fs.unlinkSync(videoPath); } catch { }
  if (clips.length === 0) return NextResponse.json({ error: "Nenhum clip gerado" }, { status: 500 });
  return NextResponse.json({ success: true, message: `${clips.length} clips gerados!`, clips, totalClips: clips.length, clipDuration, totalSeconds, format, videoSizeKB: Math.round(clips.reduce((acc, c) => acc + c.sizeKB, 0)) });
}
