  ...
  try {
    ...
    if (contentType.includes("multipart/form-data")) {
      ...
      try {
          ...
          return NextResponse.json(
            {
              ...
              detail: errText || `Status ${apiRes.status}`,
            },
            { status: 500 }
          );
        }
        ...
        const formats: RapidFormat[] =
          apiData?.formats || apiData?.adaptiveFormats || [];

        const mp4WithAudio = formats.find(
          (f) => f.mimeType?.includes("video/mp4") && f.url && f.audioQuality
        );
          ...
          return NextResponse.json(
            {
              ...
              detail: JSON.stringify(apiData).slice(0, 500),
            },
            { status: 500 }
          );
        }
    ...
    return NextResponse.json(
      { error: "Erro interno no processamento", detail: e?.message },
      { status: 500 }
    );
  }
}